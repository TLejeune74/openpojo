...
public class GenericArrayTypeResolverTest {
  ...
  public void throwsUnsupportedExceptionForGetEnclosingType() {
    try {
      ...
      assertTrue(uoe.getMessage().endsWith(") - This operation is Not Supported, if you ran into this please report this issue @ http://openpojo.com"));
    }
  }
  ...
  public void end2endTest() {
    ...
    assertNotNull(aClassWithGenericArray);
    assertTrue(aClassWithGenericArray.getMySetOfArrayElements().size() > 0);
    Object[] objects = aClassWithGenericArray.getMySetOfArrayElements().toArray();
    assertTrue(objects.length > 0);
    assertTrue(objects[0] instanceof ArrayElementsClass);
  }
